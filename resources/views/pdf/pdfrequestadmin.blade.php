...
<img src="img/logo.png" class="rounded float-end custom-image" style="width: 100px; height: 100px;">
    ...
    <section>
        <p style="text-indent: 40px; text-align: justify;">
            ...
            <strong>C. {{ $userDetails ? $userDetails->personalname : 'Nombre no disponible' }} {{ $userDetails ? $userDetails->paternallastname : '' }} {{ $userDetails ? $userDetails->maternalastname : '' }}</strong> porque una vez revisada y analizada su situación consideramos que puede desarrollar y/o fortalecer sus capacidades y competencias ahí con ustedes. Anexamos a la presente los siguientes datos:
        </p>
    </section>
            ...
            <table class="table table-bordered">
                <tbody>
                    <tr>
                        ...
                        <td class="text-start">{{ $user ? $user->email : 'Correo no disponible' }}</td>
                    </tr>
                    <tr>
                        <th class="table-active text-start">CURP:</th>
                        <td class="text-start">{{ $userDetails ? $userDetails->curp : 'CURP no disponible' }}</td>
                    </tr>
                    <tr>
                        <th class="table-active text-start">Género:</th>
                        <td class="text-start">{{ $userDetails ? $userDetails->gender : 'Género no disponible' }}</td>
                    </tr>
                    <tr>
                        <th class="table-active text-start">Número Telefónico:</th>
                        <td class="text-start">{{ $userDetails ? $userDetails->cellphonenumber : 'Número de teléfono no disponible' }}</td>
                    </tr>
                </tbody>
            </table>
        </div>
            ...
            <table class="table table-bordered">
                <tbody>
                    <tr>
                        ...
                        <td class="text-start">{{ $addressData ? $addressData->postalcode : 'Código Postal no disponible' }}</td>
                    </tr>
                    <tr>
                        <th class="table-active text-start">Municipio:</th>
                        <td class="text-start">{{ $addressData ? $addressData->municipality : 'Municipio no disponible' }}</td>
                    </tr>
                    <tr>
                        <th class="table-active text-start">Colonia:</th>
                        <td class="text-start">{{ $addressData ? $addressData->colony : 'Colonia no disponible' }}</td>
                    </tr>
                    <tr>
                        <th class="table-active text-start">Calle:</th>
                        <td class="text-start">{{ $addressData ? $addressData->street : 'Calle no disponible' }}</td>
                    </tr>
                    <tr>
                        <th class="table-active text-start">Número Exterior e Interior:</th>
                        <td class="text-start">{{ $addressData ? $addressData->outdoornumber . ' / ' . $addressData->interiornumber : 'Números no disponibles' }}</td>
                    </tr>
                </tbody>
            </table>
        </div>
    ...
    <div class="table-container">
        ...
        <div class="table-responsive">
            <table class="table table-bordered">
                <tbody>
                    <tr>
                        <th class="table-active text-start">Diagnóstico Médico:</th>
                        <td class="text-start">{{ $medicalData ? $medicalData->medicaldiagnosis : 'Diagnóstico médico no disponible' }}</td>
                    </tr>
                    <tr>
                        <th class="table-active text-start">Tipo de Sangre:</th>
                        <td class="text-start">{{ $medicalData ? $medicalData->blood_type : 'Tipo de sangre no disponible' }}</td>
                    </tr>
                    <tr>
                    ...
                    <tr>
                        <th class="table-active text-start">Enfermedades Crónicas:</th>
                        <td class="text-start">
