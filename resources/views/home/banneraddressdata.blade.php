<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <meta http-equiv="X-UA-Compatible" content="ie=edge">
    <title>Datos de Domicilio</title>
    <link rel="shortcut icon" type="image/x-icon" href="https://i.pinimg.com/564x/a6/65/26/a6652603c3ac7b1c2392274913efb673.jpg">
    <link rel="stylesheet" href="{{ asset('css/dashboard.css') }}">
    <link rel="stylesheet" href="{{ asset('css/register.css') }}">
    <link rel="stylesheet" href="{{ asset('css/profile.css') }}">
    <link rel="stylesheet" href="{{ asset('css/panel.css') }}">
     <link rel="stylesheet" href="{{ asset('css/welcome.css') }}">
     <link rel="stylesheet" href="{{ asset('css/bannerpersonaldata.css') }}">
     <link rel="stylesheet" href="https://maxcdn.bootstrapcdn.com/bootstrap/4.5.2/css/bootstrap.min.css">
     <script src="https://cdn.jsdelivr.net/npm/bootstrap@5.0.2/dist/js/bootstrap.bundle.min.js" integrity="sha384-MrcW6ZMFYlzcLA8Nl+NtUVF0sA7MsXsP1UyJoMp4YLEuNSfAP+JcXn/tWtIaxVXM" crossorigin="anonymous"></script>
    <meta http-equiv="X-UA-Compatible" content="ie=edge">
<body>
    @auth
    <nav id="navbar" class="navbar navbar-light sticky-top" style="background-color: #3498db;">
      <!--BARRA DE NAVEGACION -->

    <ul id="mn" class="nav nav-pills">
      <a class="navbar-brand" href="/welcomepanel"
          ><img src="https://i.pinimg.com/564x/a6/65/26/a6652603c3ac7b1c2392274913efb673.jpg" class="logo" alt="DEPCASWEB"></a>

          <div class="profile-container">
            <div class="profile-picture-container">
                <img src="{{ asset('storage/' . Auth::user()->profile_photo_path) }}" alt="Profile Picture">

            </div>

            <ul id="menu">
                <li><a href="">{{ auth()->user()->name ?? auth()->user()->username }}</a>
                    <ul>
                        <li><a href="/logout">Cerrar Sesión</a></li>
                    </ul>
                </li>
            </ul>
        </div>
    </nav>
    <br>
    <div class="container-fluid">
        <div class="row">
          <div class="col-md-3">
            <div class="sidebar">
              <ul class="nav flex-column">
                <li class="nav-item">
                    <a class="nav-link"  onclick="toggleCollapse('datosGenerales')">
                      <img src="https://cdn-icons-png.flaticon.com/512/1077/1077114.png" alt="Imagen" width="50" height="50">
                      Datos Generales
                    </a>
                    <div class="collapse" id="datosGenerales">
                      <div class="collapse-content">
                        <ul class="list-unstyled">
                            <li><a href="/welcomepanel/bannerpersonal" onclick="setActive(this)">
                                <img src="https://w1.pngwing.com/pngs/156/153/png-transparent-circle-silhouette-data-personally-identifiable-information-cdr-base64-black-black-and-white-text.png" alt="Imagen" width="50" height="50">
                                Datos Personales</a></li>
                            <li><a href="/welcomepanel/banneraddress" onclick="setActive(this)">
                                <img src="https://www.seekpng.com/png/full/43-431556_your-location-has-recently-been-accessed-alert-in.png" alt="Imagen" width="50" height="50">
                                Datos de Domicilio</a></li>
                            <li><a href="/welcomepanel/bannermedical" onclick="setActive(this)">
                                <img src="https://w7.pngwing.com/pngs/37/477/png-transparent-check-data-health-information-medical-report-result-infectious-pandemics-icon.png" alt="Imagen" width="50" height="50">
                                Datos Médicos</a></li>
                        </ul>
                      </div>
                    </div>
                  </li>


                <li class="nav-item">
                  <a class="nav-link"  onclick="toggleCollapse('solicitudes')">
                    <img src="https://cdn-icons-png.flaticon.com/512/7257/7257795.png" alt="Imagen" width="50" height="50">
                     Solicitudes
                    </a>
                  <div class="collapse" id="solicitudes">
                    <div class="collapse-content">
                      <ul class="list-unstyled">
                        <li><a href="/newrequest">
                      <img src="https://w7.pngwing.com/pngs/147/762/png-transparent-information-invoice-report-purchase-order-application-miscellaneous-template-angle-thumbnail.png" alt="Imagen" width="50" height="50">
                             Nueva</a></li>
                      </ul>
                    </div>
                  </div>
                </li>

                  </li>
                <li class="nav-item">
                  <a class="nav-link" href="#">
                    <img src="https://cdn.icon-icons.com/icons2/37/PNG/512/about_3697.png" alt="Imagen" width="50" height="50">
                    Acerca de
                </a>
                </li>
              </ul>
            </div>
          </div>
          <div class="col-md-9 p-3 min-vh-100 mx-auto">
            <form action="" method="POST" style="width: 90%">
                @csrf
                <div class="card">
                    <div class="card-body">
                        <h2 class="card-title" style="font-weight: bold; text-align: center;">Datos de Domicilio</h2>
                        <br>
                        <div class="row mt-2">
                            <div class="col-sm-6 custom-col">
                                <label for="postalcode" class="form-label-bold">Código Postal</label>
                                <input type="text" class="form-control" id="postalcode" name="postalcode" placeholder="Código Postal" value="{{ $addressData->postalcode }}">
                                @error('personalname')
                                <small class="txt-danger mt-1">
                                    <strong>{{ $message }}</strong>
                                </small>
                                @enderror
                            </div>
                            <div class="col-sm-6 custom-col">
                                <label for="state" class="form-label-bold">Estado</label>
                                <input type="text" class="form-control" id="state" name="state" placeholder="Estado" value="{{ $addressData->state }}">
                                @error('paternallastname')
                                <small class="txt-danger mt-1">
                                    <strong>{{ $message }}</strong>
                                </small>
                                @enderror
                            </div>
                        </div>

                        <div class="row mt-2">
                            <div class="col-sm-6 custom-col">
                                <label for="municipality" class="form-label-bold">Municipio</label>
                                <input type="text" class="form-control" id="municipality" name="municipality" placeholder="Municipio" value="{{ $addressData->municipality }}">
                                @error('maternalastname')
                                <small class="txt-danger mt-1">
                                    <strong>{{ $message }}</strong>
                                </small>
                                @enderror
                            </div>
                            <div class="col-sm-6 custom-col">
                                <label for="colony" class="form-label-bold">Colonia</label>
                                <input type="text" class="form-control" id="colony" name="colony" placeholder="Colonia" value="{{ $addressData->colony }}">
                                @error('birthdate')
                                <small class="txt-danger mt-1">
                                    <strong>{{ $message }}</strong>
                                </small>
                                @enderror
                            </div>
                        </div>

                        <div class="row mt-2">
                            <div class="col-sm-6 custom-col">
                                <label for="street" class="form-label-bold">Calle</label>
                                <input type="text" class="form-control" id="street" name="street" placeholder="Calle" value="{{ $addressData->street }}">
                                @error('curp')
                                <small class="txt-danger mt-1">
                                    <strong>{{ $message }}</strong>
                                </small>
                                @enderror
                            </div>
                            <div class="col-sm-6 custom-col">
                                <label for="interiornumber" class="form-label-bold">Número Interior</label>
                                <input type="text" class="form-control" id="interiornumber" name="interiornumber" placeholder="Número Interior" value="{{ $addressData->interiornumber }}">
                                @error('curp')
                                <small class="txt-danger mt-1">
                                    <strong>{{ $message }}</strong>
                                </small>
                                @enderror
                            </div>
                        </div>

                        <div class="row mt-2">
                            <div class="col-sm-6 custom-col">
                                <label for="outdoornumber" class="form-label-bold">Número Exterior</label>
                                <input type="text" class="form-control" id="outdoornumber" name="outdoornumber" placeholder="Número Exterior" value="{{ $addressData->outdoornumber }}">
                                @error('curp')
                                <small class="txt-danger mt-1">
                                    <strong>{{ $message }}</strong>
                                </small>
                                @enderror
                            </div>
                            <div class="col-sm-6 custom-col">
                                <label for="referencesaddress" class="form-label-bold">Referencias de Domicilio</label>
                                <input type="text" class="form-control" id="referencesaddress" name="referencesaddress" placeholder="Referencias de Domicilio" value="{{ $addressData->referencesaddress }}">
                                @error('homenumber')
                                <small class="txt-danger mt-1">
                                    <strong>{{ $message }}</strong>
                                </small>
                                @enderror
                            </div>
                        </div>

                        <br>

                        <div class="row mt-4">
                            <div class="col text-center">
                                <button type="submit" name="action" value="update" class="btn btn-primary mx-2" style="width: 200px; height: 60px; font-size: 20px;">Actualizar</button>
                                <button type="submit" name="action" value="cancel" class="btn btn-secondary mx-2" style="width: 200px; height: 60px; font-size: 20px;">Cancelar</button>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="container">
                    @if(session('success'))
                        <div class="alert alert-success" id="successMessage">
                            {{ session('success') }}
                        </div>
                    @endif

                    <!-- Resto del contenido de la página -->
                </div>
            <script>
                setTimeout(function() {
                    var successMessage = document.getElementById('successMessage');
                    if (successMessage) {
                        successMessage.style.display = 'none';
                    }
                }, 5000);
            </script>
            </form>
        </div>


    <script>
        // Función para desplegar y contraer al hacer clic
        function toggleCollapse(elementId) {
          var element = document.getElementById(elementId);
          var collapse = new bootstrap.Collapse(element);
          collapse.toggle();
        }
        function setActive(element) {
    var activeItems = document.getElementsByClassName("active");
    // Elimina la clase "active" de todos los elementos
    for (var i = 0; i < activeItems.length; i++) {
      activeItems[i].classList.remove("active");
    }
    // Agrega la clase "active" al elemento seleccionado
    element.classList.add("active");
  }
      </script>
          <br>
          <footer class="footer">
            <div class="container">
              <div class="row">
                <div class="col-md-6">
                  <img src="https://scontent.fmid1-3.fna.fbcdn.net/v/t39.30808-6/328248098_712314383727019_3805406242648634395_n.jpg?_nc_cat=100&cb=99be929b-3346023f&ccb=1-7&_nc_sid=09cbfe&_nc_eui2=AeGr55uhFWj9PoGx8MsLudtNr0bKR1TgXquvRspHVOBeq08Bh2Uz4UOdsHpx7Hpc0lgwz6CE9Vt3-NOiAk8Bg5GL&_nc_ohc=BzGgPq3uN5QAX98kCS8&_nc_ht=scontent.fmid1-3.fna&oh=00_AfBw_cI6ECZ_VRY5viHxt7Fi43yUGTgXo6gcLMo0Heavqw&oe=64A87219" alt="DIF Xoxocotlán">
                </div>
                <div class="col-md-6">
                  <img src="data:image/jpeg;base64," alt="Logo Municipio Santa Cruz Xoxocotlán">
                </div>
              </div>
              <br>
              <div class="row mt-4">
                <div class="col-md-6">
                  <div class="social-icons">
                    <a href="https://www.facebook.com/DIFXoxocotlan/?locale=es_LA" target="_blank"><i class="fab fa-facebook-f"></i></a>
                    <a href="https://twitter.com/XoxocotlanDIF" target="_blank"><i class="fab fa-twitter"></i></a>
                    <a href="#" target="_blank"><i class="fab fa-instagram"></i></a>
                  </div>
                </div>
                <div class="col-md-6 text-md-right">
                  <p>&copy; 2023 Municipio de Santa Cruz Xoxocotlán. Todos los derechos reservados.</p>
                </div>
              </div>
            </div>
          </footer>
    @endauth
    @guest
    <h1>POR FAVOR INICIA SESION PARA AUTENTIFICARSE <a href="/sesion">Iniciar Sesion</a></h1>


    @endguest
    <script src="https://cdn.jsdelivr.net/npm/@popperjs/core@2.9.2/dist/umd/popper.min.js" integrity="sha384-IQsoLXl5PILFhosVNubq5LC7Qb9DXgDA9i+tQ8Zj3iwWAwPtgFTxbJ8NT4GN1R8p" crossorigin="anonymous"></script>
    <script src="https://cdn.jsdelivr.net/npm/bootstrap@5.0.2/dist/js/bootstrap.min.js" integrity="sha384-cVKIPhGWiC2Al4u+LWgxfKTRIcfu0JTxR+EQDz/bgldoEyl4H0zUF0QKbrJ0EcQF" crossorigin="anonymous"></script>
    <script src="https://ajax.googleapis.com/ajax/libs/jquery/3.5.1/jquery.min.js"></script>
  <script src="https://cdnjs.cloudflare.com/ajax/libs/popper.js/1.16.0/umd/popper.min.js"></script>
  <script src="https://maxcdn.bootstrapcdn.com/bootstrap/4.5.2/js/bootstrap.min.js"></script>
</body>
</html>
