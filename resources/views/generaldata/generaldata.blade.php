<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <meta http-equiv="X-UA-Compatible" content="ie=edge">
    <title>Datos Generales</title>
    <link rel="shortcut icon" type="image/x-icon" href="https://i.pinimg.com/564x/a6/65/26/a6652603c3ac7b1c2392274913efb673.jpg">
    <link rel="stylesheet" href="{{ asset('css/dashboard.css') }}">
    <link rel="stylesheet" href="{{ asset('css/register.css') }}">
    <link rel="stylesheet" href="{{ asset('css/profile.css') }}">
    <link rel="stylesheet" href="{{ asset('css/generaldata.css') }}">
   <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.0.2/dist/css/bootstrap.min.css" rel="stylesheet" integrity="sha384-EVSTQN3/azprG1Anm3QDgpJLIm9Nao0Yz1ztcQTwFspd3yD65VohhpuuCOmLASjC" crossorigin="anonymous">
    <script src="https://cdn.jsdelivr.net/npm/bootstrap@5.0.2/dist/js/bootstrap.bundle.min.js" integrity="sha384-MrcW6ZMFYlzcLA8Nl+NtUVF0sA7MsXsP1UyJoMp4YLEuNSfAP+JcXn/tWtIaxVXM" crossorigin="anonymous"></script>
    <meta http-equiv="X-UA-Compatible" content="ie=edge">

</head>
<body>
    @auth
    <nav id="navbar" class="navbar navbar-light sticky-top" style="background-color: #3498db;">
      <!--BARRA DE NAVEGACION -->

    <ul id="mn" class="nav nav-pills">
      <a class="navbar-brand" href="index.html"
          ><img src="https://i.pinimg.com/564x/a6/65/26/a6652603c3ac7b1c2392274913efb673.jpg" class="logo" alt="DEPCASWEB"></a>

          <div class="profile-container">
            <div class="profile-picture-container">
                <img src="{{ asset('storage/' . Auth::user()->profile_photo_path) }}" alt="Profile Picture">

            </div>

            <ul id="menu">
                <li><a href="">{{ auth()->user()->name ?? auth()->user()->username }}</a>
                    <ul>
                        <li><a href="/logout">Cerrar Sesión</a></li>
                    </ul>
                </li>
            </ul>
        </div>
    </nav>
    <form action="" method="get" class="m-auto bg-white p-5 rounded-sm shadow-lg w-form ">
        @csrf
        <h2 class="text-center">DATOS GENERALES</h2>
        <br>
        <h2 class="text-center">DATOS PERSONALES</h2>
        @foreach ($userDetails as $userDetail)
            <p class="data-heading">Nombre:</p>
            <p class="data-value">{{ $userDetail->personalname }}</p>
            <p class="data-heading">Apellido Paterno:</p>
            <p class="data-value">{{ $userDetail->paternallastname }}</p>
            <p class="data-heading">Apellido Materno:</p>
            <p class="data-value">{{ $userDetail->maternalastname }}</p>
            <p class="data-heading">Fecha de Nacimiento:</p>
            <p class="data-value">{{ \Carbon\Carbon::parse($userDetail->birthdate)->format('d/m/Y') }}</p>
            <p class="data-heading">CURP:</p>
            <p class="data-value">{{ $userDetail->curp }}</p>
            <p class="data-heading">Género:</p>
            <p class="data-value">{{ $userDetail->gender }}</p>
            <p class="data-heading">Teléfono Móvil:</p>
            <p class="data-value">{{ $userDetail->cellphonenumber }}</p>
            <p class="data-heading">Teléfono de Casa:</p>
            <p class="data-value">{{ $userDetail->homenumber }}</p>
            <!-- Mostrar otros campos de user_details -->
        @endforeach

        <!-- Mostrar los datos de address_data -->
        <br>
        <h2 class="text-center">DATOS DE DOMICILIO</h2>
        @foreach ($addressData as $address)
            <p class="data-heading">Código Postal:</p>
            <p class="data-value">{{ $address->postalcode }}</p>
            <p class="data-heading">Estado:</p>
            <p class="data-value">{{ $address->state }}</p>
            <p class="data-heading">Municipio:</p>
            <p class="data-value">{{ $address->municipality }}</p>
            <p class="data-heading">Colonia:</p>
            <p class="data-value">{{ $address->colony }}</p>
            <p class="data-heading">Calle:</p>
            <p class="data-value">{{ $address->street }}</p>
            <p class="data-heading">Número Interior:</p>
            <p class="data-value">{{ $address->interiornumber }}</p>
            <p class="data-heading">Número Exterior:</p>
            <p class="data-value">{{ $address->outdoornumber }}</p>
            <p class="data-heading">Referencias Geográficas:</p>
            <p class="data-value">{{ $address->referencesaddress }}</p>
            <!-- Mostrar otros campos de address_data -->
        @endforeach

        <!-- Mostrar los datos de medical_data -->
        <br>
        <h2 class="text-center">DATOS MÉDICOS</h2>
        @foreach ($medicalData as $medical)
            <p class="data-heading">Diagnóstico Médico:</p>
            <p class="data-value">{{ $medical->medicaldiagnosis }}</p>
            <p class="data-heading">Tipo de Sangre:</p>
            <p class="data-value">{{ $medical->blood_type }}</p>
            <p class="data-heading">Tipo de Alergia:</p>
            <p class="data-value">{{ $medical->allergies_type }}</p>
            <p class="data-heading">Nombre de la Alergia:</p>
            <p class="data-value">{{ $medical->allergies }}</p>
            <p class="data-heading">Consideraciones Médicas Adicionales:</p>
            <p class="data-value">{{ $medical->additionaldescription }}</p>
            <p class="data-heading">Tipo de Enfermedad:</p>
            <p class="data-value">{{ $medical->chronic_diseases }}</p>
            <!-- Mostrar otros campos de medical_data -->
            @endforeach
<br>
<div id="btnrg">
    <a href="{{ route('welcomepanel') }}" class="btn btn-primary btn-lg">Finalizar</a>
</div>

 @endauth
        @guest
        <h1>POR FAVOR INICIA SESION PARA AUTENTIFICARSE <a href="/sesion">Iniciar Sesion</a></h1>
        <br>
        <footer class="footer">
          <div class="container">
            <div class="row">
              <div class="col-md-6">
                <img src="https://scontent.fmid1-3.fna.fbcdn.net/v/t39.30808-6/328248098_712314383727019_3805406242648634395_n.jpg?_nc_cat=100&cb=99be929b-3346023f&ccb=1-7&_nc_sid=09cbfe&_nc_eui2=AeGr55uhFWj9PoGx8MsLudtNr0bKR1TgXquvRspHVOBeq08Bh2Uz4UOdsHpx7Hpc0lgwz6CE9Vt3-NOiAk8Bg5GL&_nc_ohc=k512WFKozNgAX9pQAqP&_nc_ht=scontent.fmid1-3.fna&oh=00_AfBZEz0gqcW57qV0_lVir1NUI5JaA0TUhNeFlGv-Ah_Wvw&oe=64A28359" alt="DIF Xoxocotlán">
              </div>
              <div class="col-md-6">
                <img src="data:image/jpeg;base64," alt="Logo Municipio Santa Cruz Xoxocotlán">
              </div>
            </div>
            <br>
            <div class="row mt-4">
              <div class="col-md-6">
                <div class="social-icons">
                  <a href="https://www.facebook.com/DIFXoxocotlan/?locale=es_LA" target="_blank"><i class="fab fa-facebook-f"></i></a>
                  <a href="https://twitter.com/XoxocotlanDIF" target="_blank"><i class="fab fa-twitter"></i></a>
                  <a href="#" target="_blank"><i class="fab fa-instagram"></i></a>
                </div>
              </div>
              <div class="col-md-6 text-md-right">
                <p>&copy; 2023 Municipio de Santa Cruz Xoxocotlán. Todos los derechos reservados.</p>
              </div>
            </div>
          </div>
        </footer>

        @endguest
        <script src="https://cdn.jsdelivr.net/npm/@popperjs/core@2.9.2/dist/umd/popper.min.js" integrity="sha384-IQsoLXl5PILFhosVNubq5LC7Qb9DXgDA9i+tQ8Zj3iwWAwPtgFTxbJ8NT4GN1R8p" crossorigin="anonymous"></script>
        <script src="https://cdn.jsdelivr.net/npm/bootstrap@5.0.2/dist/js/bootstrap.min.js" integrity="sha384-cVKIPhGWiC2Al4u+LWgxfKTRIcfu0JTxR+EQDz/bgldoEyl4H0zUF0QKbrJ0EcQF" crossorigin="anonymous"></script>
    </body>
    </html>
