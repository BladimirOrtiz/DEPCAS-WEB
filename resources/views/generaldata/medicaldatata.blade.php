    ...
    <nav id="navbar" class="navbar navbar-light sticky-top" style="background-color: #3498db;">
      ...
      <ul id="mn" class="nav nav-pills">
        <a class="navbar-brand" href="index.html"
            ><img src="https://i.pinimg.com/564x/a6/65/26/a6652603c3ac7b1c2392274913efb673.jpg" class="logo" alt="DPCASWEB"></a>
           </ul>
          <ul id="menu">
...
<form action="" method="post" class="m-auto bg-white p-5 rounded-sm shadow-lg w-form ">
    @csrf
        ...
        <div class="form-group"> <!-- Tipo de Sangre -->
            <label for="exampleInputName" class="control-label">Tipo de Sangre</label>
            <input type="text" class="form-control" id="blood_type" name="blood_type" placeholder="Tipo de Sangre" value="{{ old('blood_type') }}">
            @error('medicaldata')
            <small class="txt-danger mt-1">
                <strong>{{ $message }}</strong>
            </small>
        ...
        <div class="form-group"> <!-- ¿Padeces de Una Alergía? -->
            <label for="exampleInputName" class="control-label">¿Padeces de Una Alergía?</label>
            <input type="radio" name="opcion" value="1" id="opcion_si" onclick="habilitarSelect();" {{ old('opcion') == '1' ? 'checked' : '' }} />
            <label for="opcion_si">Sí</label>
            <input type="radio" name="opcion" value="0" id="opcion_no" onclick="deshabilitarSelect();" {{ old('opcion') == '0' ? 'checked' : '' }} />
        ...
        <div class="form-group"> <!-- Tipo de Alergia -->
            <label for="exampleInputName" class="control-label">Tipo de Alergia</label>
            <select class="form-select" aria-label="Default select example" name="allergies_type" id="select_allergies" {{ old('opcion') == '1' ? '' : 'disabled' }}>
                <option selected>Selecciona</option>
                <option value="Alergia a Alimentos" {{ old('allergies_type') == 'Alergia a Alimentos' ? 'selected' : '' }}>Alergia a Alimentos</option>
                <option value="Alergia a Fármacos" {{ old('allergies_type') == 'Alergia a Fármacos' ? 'selected' : '' }}>Alergia a Fármacos</option>
                <option value="Rinitis Alérgica" {{ old('allergies_type') == 'Rinitis Alérgica' ? 'selected' : '' }}>Rinitis Alérgica</option>
                <option value="Otro" {{ old('allergies_type') == 'Otro' ? 'selected' : '' }}>Otro</option>
            </select>
            @error('medicaldata')
            <small class="txt-danger mt-1">
                <strong>{{ $message }}</strong>
        ...
        <div class="form-group"> <!-- Nombre de la Alergia -->
            <label for="exampleInputName" class="control-label">Nombre de la Alergia</label>
            <input type="text" class="form-control" id="allergies" name="allergies" placeholder="Nombre de la Alergia" value="{{ old('allergies') }}">
            @error('medicaldata')
            <small class="txt-danger mt-1">
                <strong>{{ $message }}</strong>
            </small>
        ...
        <div class="form-group"> <!-- Consideraciones Médicas Adicionales -->
            <label for="address_references">Consideraciones Médicas Adicionales</label>
            <textarea id="address_references" name="additionaldescription" class="form-control" placeholder="Consideraciones Médicas Adicionales" rows="4">{{ old('additionaldescription') }}</textarea>
        ...
        <div class="form-group"> <!-- ¿Padeces una Enfermedad Crónica? -->
            <label for="exampleInputName" class="control-label">¿Padeces una Enfermedad Crónica?</label>
            <input type="radio" name="opcion" value="1" id="opcion_si" onclick="habilitarSelectcro();" {{ old('opcion') == '1' ? 'checked' : '' }} />
            <label for="opcion_si">Sí</label>
            <input type="radio" name="opcion" value="0" id="opcion_no" onclick="deshabilitarSelectcro();" {{ old('opcion') == '0' ? 'checked' : '' }} />
            <label for="opcion_no">No</label>
            @error('medicaldata')
            <small class="txt-danger mt-1">
                <strong>{{ $message }}</strong>
        ...
        <div class="form-group"> <!-- Tipo de Enfermedad Crónica -->
            <label for="exampleInputName" class="control-label">Tipo de Enfermedad Crónica</label>
            <input type="text" class="form-control" name="chronic_diseases" id="select_chronic_diseases" placeholder="Tipo de Enfermedad Crónica" {{ old('opcion') == '1' ? '' : 'disabled' }} value="{{ old('chronic_diseases') }}">
            @error('medicaldata')
            <small class="txt-danger mt-1">
                <strong>{{ $message }}</strong>
            </small>
      ...
      <script>
        function habilitarSelect() {
            document.getElementById("select_allergies").disabled = false;
            document.getElementById("bloodtype").disabled = false;

        }

        function deshabilitarSelect() {
            document.getElementById("select_allergies").disabled = true;
            document.getElementById("bloodtype").disabled = true;
        }
        function habilitarSelectcro(){
            document.getElementById("select_chronic_diseases").disabled = false;
        }
        function deshabilitarSelectcro(){
            document.getElementById("select_chronic_diseases").disabled = true;
    ...
    <footer class="footer">
      <div class="container">
        <div class="row">
          <div class="col-md-6">
            <img src="https://scontent.fmid1-3.fna.fbcdn.net/v/t39.30808-6/328248098_712314383727019_3805406242648634395_n.jpg?_nc_cat=100&cb=99be929b-3346023f&ccb=1-7&_nc_sid=09cbfe&_nc_eui2=AeGr55uhFWj9PoGx8MsLudtNr0bKR1TgXquvRspHVOBeq08Bh2Uz4UOdsHpx7Hpc0lgwz6CE9Vt3-NOiAk8Bg5GL&_nc_ohc=k512WFKozNgAX9pQAqP&_nc_ht=scontent.fmid1-3.fna&oh=00_AfBZEz0gqcW57qV0_lVir1NUI5JaA0TUhNeFlGv-Ah_Wvw&oe=64A28359" alt="DIF Xoxocotlán">
          </div>
          <div class="col-md-6">
